...
const STATUS_CONFIG: Record<DocumentStatus, StatusConfig> = {
  'not-started': { bg: 'bg-glass-border', symbol: '' },
...
function DocItem({ docId, onOpenDoc }: DocItemProps) {
  ...
  return (
    ...
    >
      <span
        className={`w-7 h-7 rounded-full flex items-center justify-center text-caption font-bold text-white ${cfg.bg}`}
        aria-hidden
      >
        {cfg.symbol}
      </span>
      ...
      <div className="max-w-container mx-auto px-4 py-8 flex flex-col gap-6">
        ...
        <Glass className="p-6 flex flex-col gap-4" data-testid="track-header">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h1 className="text-h2 font-display text-text-primary">{t(track.titleKey)}</h1>
